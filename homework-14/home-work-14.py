...
while True:
    ...
    if key == "add":
        ...
        if name in dic:
            ...
            number = input("Нове ім'я контакту записано. Введіть номер нового контакту у одному із форматів: +380XXXXXXXXX, 380XXXXXXXXX, 0XXXXXXXXX")
            if re.fullmatch(r'\+380\d{9,9}', number):
                aprooved_number = number
                add(name, aprooved_number)
            elif re.fullmatch(r'380\d{9,9}', number):
                aprooved_number = number
                add(name, aprooved_number)
            elif re.fullmatch(r'0\d{9,9}', number):
                aprooved_number = number
                add(name, aprooved_number)
            else:
                print("Введено неправильний формат номера телефону. Спробуйте додати новий контакт спочатку")
                continue
